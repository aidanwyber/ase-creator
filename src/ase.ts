...
export class ASE {
  ...
  static fromColors(colors: ASEColor[], options?: Omit<ASECreateOptions, "colors">): ASE {
  ...
  addGradient(stops: ASEGradientStop[], options?: ASEGradientOptions): this {
    ...
    const safeSteps =
      Number.isFinite(options?.steps) ? clamp(Math.round(options?.steps ?? 7), 2, 64) : 7;
  ...
  download(filename = "swatches.ase"): void {
    if (typeof document === "undefined" || typeof URL === "undefined") {
      throw new Error("ASE.download() is only available in browser environments.");
    }
    if (typeof URL.createObjectURL !== "function" || typeof URL.revokeObjectURL !== "function") {
      throw new Error("Browser URL APIs are not available for downloading.");
    }

    const safeFilename = filename.endsWith(".ase") ? filename : `${filename}.ase`;
