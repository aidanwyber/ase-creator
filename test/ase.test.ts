...
const parseAse = (bytes: Uint8Array): ParsedAse => {
  ...
  for (let i = 0; i < blockCount; i += 1) {
    ...
    if (blockType === 0xc001) {
      ...
      const groupName = decodeUtf16Be(bytes.slice(offset, offset + nameBytesLength)).replace(
        /\u0000$/,
        ""
      );
    ...
    if (blockType === 0x0001) {
      ...
      offset += 2;
      const nameBytesLength = nameLength * 2;
      const swatchName = decodeUtf16Be(bytes.slice(offset, offset + nameBytesLength)).replace(
        /\u0000$/,
        ""
      );
      ...
      offset += 4;
      if (mode !== "RGB") {
        throw new Error(`Unsupported swatch mode in test parser: ${mode}`);
      }
